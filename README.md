...
- 修复账户管理中重置密码请求体格式错误，恢复前端点击重置密码后的正常调用。
- Docker Compose 补充 `JWT_SECRET` 与 `.env` 加载配置，降低生产启动时遗漏密钥的风险。

### v1.1.5

- 整理文档结构，移除 README 中的发布流程段落。
- 新增更安全的登录页默认管理员提示文案，不再直接展示默认口令。
- 增补升级注意事项与版本记录，降低发布说明分散在 GitHub release 中的维护成本。
- 将固定发布要求沉淀到仓库级代理指引，便于后续版本继续保持一致。
